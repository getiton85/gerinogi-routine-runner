...
function Ensure-BundledSkipSample {
    try {
        ...
        $target = Join-Path $script:SampleDir '스킵_마스크테스트.png'
...
function Add-MaintenanceLogLine([string]$Line) {
    try {
        $path = Join-Path $PSScriptRoot 'maintenance_log.txt'
        [System.IO.File]::AppendAllText($path, ((Get-Date).ToString('s') + ' ' + $Line + [Environment]::NewLine), [System.Text.Encoding]::UTF8)
    } catch { }
}
...
function Invoke-MaintenanceCleanup {
    ...
    Add-MaintenanceLogLine ('cleanup diagnostic_png=' + $removedDiag + '; report_zip=' + $removedReports + '; report_dirs=' + $removedReportDirs + '; update_backup_dirs=' + $removedUpdateBackups + '; install_backup_dirs=' + $removedInstallBackups)
}
...
function Click-SlotTarget([string]$Slot, [System.Drawing.Rectangle]$Rect, [int]$DelayMs, [int]$HoldOverrideMs = -1) {
    ...
    if (-not (Test-SlotAllowsCoordinateFallback $Slot)) {
        Write-RoutineTrace $script:CurrentCycle 'click-target' $Slot 'coordinate-fallback-blocked' ([System.Drawing.Rectangle]::Empty) 'image miss; coordinate fallback is disabled for this slot'
        return $false
    }
...
function Test-SlotRequiresRegion([string]$Slot) {
    ...
    return @('협동','메뉴','어비스','던전','입장','완료 확인','나가기','식사 버튼','궁극기','스킵','팔라딘') -contains $baseSlot
}
function Test-SlotAllowsCoordinateFallback([string]$Slot) {
    if ($script:SpecialSlots -contains $Slot) { return $false }
    $baseSlot = Resolve-RouteSlotFromStorageKey $Slot
    return (@($script:RouteSlots) | Select-Object -First 4) -contains $baseSlot
}
    ...
    foreach ($samplePath in $paths) {
        ...
        if ($Slot -eq '스킵' -or $Slot -eq '완료 확인') {
            ...
            if (-not $brightRect.IsEmpty) {
                ...
                if ($Slot -eq '스킵' -and [VisionFinder]::LastAmbiguous) {
...
function Find-CoopPromptStrict([System.Windows.Forms.Screen]$Screen) {
    ...
    Write-RoutineTrace $script:CurrentCycle 'stage-scan' '협동' 'guard-found-button-miss-no-click' $guardRect ('guard=' + $guardName + '; button sample miss; point fallback disabled')
    return [System.Drawing.Rectangle]::Empty
}
    ...
    if ($Stage -in @('메뉴','메뉴확인')) {
        if ((Test-SpecialSlotEnabled '협동') -and (Get-SlotSamplePaths '협동').Count -gt 0 -and (Test-CoopAttemptAllowed)) {
            ...
            if (-not $priorityCoopRect.IsEmpty) {
                $script:PendingCompleteSeen = 0
                Write-RoutineTrace $script:CurrentCycle 'stage-scan' '협동' 'candidate-priority-special' $priorityCoopRect ('stage=' + $Stage + '; coop prompt priority')
                return [pscustomobject]@{ Slot = '협동'; Rect = $priorityCoopRect; Stage = $Stage }
            }
    ...
    if ($Stage -eq '내부') {
        ...
        if ($stateRect.IsEmpty -and (Test-SpecialSlotEnabled '협동') -and (Get-SlotSamplePaths '협동').Count -gt 0) {
            ...
            if (-not $coopInsideRect.IsEmpty) {
                $script:PendingCompleteSeen = 0
                Write-RoutineTrace $script:CurrentCycle 'stage-scan' '협동' 'blocked-inside-special' $coopInsideRect 'inside stage keeps current flow; coop does not advance to menu'
            } else {
                Write-RoutineTrace $script:CurrentCycle 'stage-scan' '협동' 'miss-inside-special' ([System.Drawing.Rectangle]::Empty) 'state marker not visible; coop not visible'
            }
        }
        if ($stateRect.IsEmpty) {
        ...
        $script:PendingCompleteSeen = 0
        Write-RoutineTrace $script:CurrentCycle 'stage-scan' '' 'none' ([System.Drawing.Rectangle]::Empty) 'stage=내부; checked=상태 기준|스킵|협동|식사 버튼|궁극기|팔라딘|완료 확인'
        return $null
    }
    $expectedSlot = $Stage
...
function Invoke-RoutineCandidateAction($Candidate, [System.Windows.Forms.Screen]$Screen, [System.Windows.Forms.Label]$StatusLabel, [ref]$InsidePhase) {
    ...
    switch ($slot) {
        ...
        '협동' {
            ...
            if ([string]$Candidate.Stage -eq '내부') {
                $nextStage = '내부'
            } else {
...
function Get-SelectedQuestProfile {
    $index = [Math]::Max(0, [Math]::Min($script:QuestProfiles.Count - 1, [int]$script:SelectedQuestProfileIndex))
    return $script:QuestProfiles[$index]
}
...
function Save-UserSettings {
    try {
        Save-SelectedUltimateProfileFromControls
        $specialEnabled = [ordered]@{}
        foreach ($slot in $script:SpecialSlots) { $specialEnabled[$slot] = [bool](Test-SpecialSlotEnabled $slot) }
    ...
    try {
        ...
        if ($settings.special_enabled) {
            foreach ($slot in $script:SpecialSlots) {
                ...
                if ($null -ne $prop) { $script:SpecialSlotEnabled[$slot] = [bool]$prop.Value }
            }
        }
...
$questProfileBox = New-Object System.Windows.Forms.ComboBox; $questProfileBox.DropDownStyle = 'DropDownList'; $questProfileBox.Dock = 'Fill'
$slotSelectTable.Controls.Add($slotBox, 0, 0)
...
$routeHeaderPanel = New-Object System.Windows.Forms.TableLayoutPanel; $routeHeaderPanel.Dock = 'Fill'; $routeHeaderPanel.ColumnCount = 2; $routeHeaderPanel.RowCount = 1
...
$harborPreviewFoldButton = New-Object System.Windows.Forms.Button; $harborPreviewFoldButton.Text = '접기'; $harborPreviewFoldButton.Dock = 'Fill'
...
$routePreviewTable.Controls.Add($routeHeaderPanel, 0, 0)
...
$cavePreviewTable.Controls.Add($caveHeaderPanel, 0, 0)
...
$combatTogglePanel.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Absolute, 66))) | Out-Null
...
$combatPreviewFoldButton = New-Object System.Windows.Forms.Button; $combatPreviewFoldButton.Text = '접기'; $combatPreviewFoldButton.Dock = 'Fill'
...
function Get-SlotPreviewOpenHeight {
    if ($script:SlotPreviewCollapsed) { return 56 }
    $height = 26 + 78
    $height += if ($script:HarborPreviewCollapsed) { 30 } else { 118 }
    $height += if ($script:CavePreviewCollapsed) { 30 } else { 118 }
    $height += if ($script:CombatPreviewCollapsed) { 34 } else { 150 }
    return ($height + 18)
}
function Update-SlotPreviewCollapsed {
    ...
    $routeSlotPanel.Visible = $groupVisible -and (-not $script:HarborPreviewCollapsed)
    ...
    $slotPreviewTable.RowStyles[1].Height = if ($groupVisible) { 78 } else { 0 }
    ...
    $harborPreviewFoldButton.Text = if ($script:HarborPreviewCollapsed) { '열기' } else { '접기' }
    $cavePreviewFoldButton.Text = if ($script:CavePreviewCollapsed) { '열기' } else { '접기' }
    $combatPreviewFoldButton.Text = if ($script:CombatPreviewCollapsed) { '열기' } else { '접기' }
...
function Toggle-HarborPreview {
    $script:HarborPreviewCollapsed = -not $script:HarborPreviewCollapsed
    Update-SlotPreviewCollapsed
}
    ...
    try {
        foreach ($slot in $script:SpecialSlots) {
            if ($script:SpecialSlotChecks.ContainsKey($slot)) {
                $script:SpecialSlotChecks[$slot].Checked = [bool]$script:SpecialSlotEnabled[$slot]
            }
        }
    } catch { }
    try {
...
$questProfileBox.Add_SelectedIndexChanged({
    ...
    Refresh-Slots
    $statusLabel.Text = '퀘스트 설정 ' + ([int]$script:SelectedQuestProfileIndex + 1) + ' 선택됨. F8로 이미지를 등록하세요.'
})
...
$harborPreviewFoldButton.Add_Click({ Toggle-HarborPreview })
...
function Start-StateRoutine {
    ...
    if (-not (Test-AnyDungeonRoutineEnabled)) { [System.Windows.Forms.MessageBox]::Show('실행할 던전 루틴이 모두 OFF 상태입니다. 허상의 정박지 또는 광기의 동굴 ON을 체크하세요.', '실행') | Out-Null; return }
